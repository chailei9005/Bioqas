...
//  val password = "test"
  ...
  val sql = s"MERGE INTO ${Configs.getString(Constants.DB_DATABASE)}.TP_E_BD_GROUP_STATISTICS_PP t1 USING (SELECT ? ID ,? CENTER_ID, ? PLAN_ID, ? SPEC_ID," +
