...
//  val password = "test"
  ...
  val sql = s"MERGE INTO ${Configs.getString(Constants.DB_DATABASE)}.TP_E_BD_Target_pec_lab t1 USING (SELECT ? ID ,? CENTER_ID, ? PLAN_ID, ? SPEC_ID,? ITEM_ID," +
