...
import com.bioqas.test.{MiddleValueUDAF, OracleDialect}
...
//          .set("spark.sql.shuffle.partitions", "10")
    ...
    val paramMap = Map[String, Object](
      ...
      "group.id" -> "test_group_progress", // 原来是test  ------ 测试group test_group_progress
      ...
      if (rdd != null) {
        ...
        if (!"".equals(idAcc.value)) {
          ...
          for (x: String <- ids.toSet) {
            ...
            //groupId.append("group_id=").append("693")
            ...
            //            evaluate.put("6931104804",evaluateArray(1) + "," + evaluateArray(2))
          ...
          val phoenixDF = ss.sqlContext.phoenixTableAsDataFrame(
            ...
            Some("biaoyuan01,biaoyuan02,biaoyuan03:2181")
          ) //.cache()
          ...
          val df = ss.sql("select tp.*,tt.mn,tt.sd,tt.middle from tp join (select CENTER_ID,PLAN_ID,SPEC_ID,ITEM_ID," +
            "TIMES_ID,BATCH_ID,GROUP_ID,GROUP_NAME,BATCH_NAME,count(BATCH_ID),mean(REPORT_DATA) as mn,middleValue(REPORT_DATA) as middle,stddev(REPORT_DATA) " +
            "as sd from tp group by CENTER_ID,PLAN_ID,SPEC_ID,ITEM_ID,TIMES_ID,BATCH_ID,GROUP_ID,GROUP_NAME,BATCH_NAME) tt on " +
          ...
          val broadcastBatchWj: Broadcast[Dataset[Row]] = ss.sparkContext.broadcast(batchWjDF)

//          val batchWjCount = ss.sparkContext.longAccumulator("batchWjCount")

//          val resultBatch = new IdAccumulator

//          broadcastBatchWj.value.show()

//          ss.sparkContext.register(resultBatch, "resultBatch")

          println("count = " + wjTotal2.count())
          println("distinct count = " + wjTotal2.distinct().count())

          wjTotal2.repartition(8).foreachPartition(rows => {
...
//            batchWjCount.add(1L)
            ...
            if (batchWj.toSet.contains(-2)) {

//              broadcastBatchWj.value.show()
              broadcastBatchWj.value.foreachPartition(row => {
                ...
                connect.commit()
                OracleGroupPPDbUtils.close(connect, pstmt)
...
//          println("value:" + batchWjCount.value)
//
//          batchWjCount.reset()
//          resultBatch.reset()
          ...
          val targetRDD = mmRdd.rdd.map(line => {
            ...
            ((line.getAs[Integer](0), line.getAs[Integer](1), line.getAs[Integer](2), line.getAs[Integer](3),
              line.getAs[Integer](4), line.getAs[Integer](5), line.getAs[Integer](6), line.getAs[String](9), line.getAs[String](11)),
          ...
          val broadcastLab: Broadcast[Dataset[Row]] = ss.sparkContext.broadcast(batchLabDF)


          println("count = " + scoreDF.count())
          println("distinct count = " + scoreDF.distinct().count())

          scoreDF.repartition(8).foreachPartition(rows => {
...
//            batchWjCount.add(1L)
            ...
            batchLab.foreach(println)


            if (batchLab.toSet.contains(-2)) {

//              broadcastLab.value.show()
              broadcastLab.value.foreachPartition(row => {
...
//          resultBatch.reset()
          ...
          val broadcastPercent: Broadcast[Dataset[Row]] = ss.sparkContext.broadcast(batchPercentDF)

          println("count = " + resultDF.count())
          println("distinct count = " + resultDF.distinct().count())


          resultDF.repartition(8).foreachPartition(rows => {
...
//            batchWjCount.add(1L)
            ...
            batchPercent.foreach(println)


            if (batchPercent.toSet.contains(-2)) {

//              broadcastPercent.value.show()
              broadcastPercent.value.foreachPartition(row => {
