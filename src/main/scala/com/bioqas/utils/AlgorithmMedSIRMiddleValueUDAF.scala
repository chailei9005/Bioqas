...
/**
  ...
  */
class AlgorithmMedSIRMiddleValueUDAF extends UserDefinedAggregateFunction{
...
//    println("////////////// " + intArray)
    val list = intArray.split(",").map(_.toDouble)
    val len = list.size
    val statisticsSQ: RetRobustObj = AlgorithmMedSIR.RobustStatisticsSQ(list)

    if(statisticsSQ == null){
      null
    }else{
      statisticsSQ.getRobustMean
    }
